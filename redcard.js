...
.controller("TestsCtrl", function($scope, $http, $cookies, $window) {
    ...
    $scope.user_isAdmin = function() {
        if ($scope.logged_user[0].user_function == "Administrador")
            return 1;
    ...
    $scope.user_isSuper = function() {
        if (($scope.logged_user[0].user_function == "Supervisor") || ($scope.logged_user[0].user_function == "Administrador"))
            return 1;
    ...
    $scope.getRedCard = function(redCardId) {
        $http.get("/checklist_site/get_red_card/" + redCardId)
        .success(function(data){
            $scope.redCard.cause = data.tests[0]['cause']
            $scope.redCard.checklist = data.tests[0]['checklist']
            $scope.redCard.code = data.tests[0]['code']
            $scope.redCard.custs = data.tests[0]['custs']
            $scope.redCard.data = data.tests[0]['data']
            $scope.redCard.description = data.tests[0]['description']
            $scope.redCard.id = data.tests[0]['id']
            $scope.redCard.status = data.tests[0]['status']
            $scope.redCard.step_order = data.tests[0]['step_order']
            $scope.redCard.test = data.tests[0]['test']
            $scope.redCard.text = data.tests[0]['text']
            $scope.redCard.time = data.tests[0]['time']
    ...
    $scope.GetAllTests = function(page) {
        ...
        request.push({
            ...
            redCard: 1,
            informationText: $scope.informationText
        });

        url = "/checklist_site/tests_get_red_card/"
        console.log(request)
        ...
        success(function(data, status, headers, config) {
            ...
            for (i = 0; i < data_tests.length; i++){
                $scope.tests_list.push({
                    ...
                    redCardId: data_tests[i].checklist_redcard_id
                });
            }
